...
const GetSymbolsInput = z.object({
  query: z.string().optional().describe("Text filter (e.g. 'BTC', 'gold', 'AAPL')"),
  asset_type: z
    .enum(ASSET_TYPES)
    .optional()
    .describe(
...
export function registerGetSymbols(
  ...
  server.registerTool(
    ...
    async (params) => {
      ...
      try {
        let feeds = await historyClient.getSymbols(undefined, params.asset_type);
        ...
        logToolCall(logger, "get_symbols", "success", Date.now() - start, false);
        return {
          content: [{ type: "text" as const, text: JSON.stringify(result) }],
        };
      } catch (err) {
        logger.warn({ err }, "get_symbols upstream error");
        logToolCall(logger, "get_symbols", "error", Date.now() - start, false, "upstream");
