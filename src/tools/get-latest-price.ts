...
const GetLatestPriceInput = z.object({
  symbols: z
    .array(z.string())
    .max(100)
    .optional()
...
export function registerGetLatestPrice(
  ...
  server.registerTool(
    ...
    async (params) => {
      ...
      if (!(params.symbols?.length ?? 0) && !(params.price_feed_ids?.length ?? 0)) {
        logToolCall(logger, "get_latest_price", "error", Date.now() - start, false, "validation");
        return toolError("At least one of 'symbols' or 'price_feed_ids' is required");
      }

      if ((params.symbols?.length ?? 0) + (params.price_feed_ids?.length ?? 0) > 100) {
        logToolCall(logger, "get_latest_price", "error", Date.now() - start, false, "validation");
        return toolError("Combined total of symbols and price_feed_ids must not exceed 100");
      }

      if (!params.access_token) {
        logToolCall(logger, "get_latest_price", "error", Date.now() - start, false, "auth");
      ...
      try {
        ...
        const enriched = feeds.map(addDisplayPrices);

        logToolCall(logger, "get_latest_price", "success", Date.now() - start, true);
        return {
          content: [
        ...
        logToolCall(logger, "get_latest_price", "error", Date.now() - start, true, errorType);
