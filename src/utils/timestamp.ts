...
export function normalizeTimestampToMicroseconds(ts: number): number {
  ...
  if (digits <= 13) return ts * 1_000; // milliseconds
 ...
 */
export function alignTimestampToChannel(timestampUs: number, channel: string): number {
  const match = channel.match(/fixed_rate@(\d+)ms/);
  if (!match) return timestampUs;
  const intervalUs = parseInt(match[1], 10) * 1000;
